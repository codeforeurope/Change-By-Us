...
class Resource(Controller):
    def GET(self, action=None, param0=None):
        is_logged_in = self.require_login("/login")
    ...
    def addResource(self):
        ...
        keywords = ' '.join([word.strip() for word in self.request('keywords').split(',')]) if not util.strNullOrEmpty(
            self.request('keywords')) else None
