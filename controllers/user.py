...
class UserAccount(Controller):
    ...
    def showAccountPage(self):
        if (self.user and self.user.data):
            ...
            return self.render('useraccount',
                               {'connected_fb': connected_fb, 'connected_tw': connected_tw, 'test': True})
        else:
    ...
    def editUser(self):
        ...
        if (not util.strNullOrEmpty(firstName) and
            ...
            if locationId == 'null':  # It may happen that the parameter is passed as the 'null' string
                return self.user.updateInfo(self.user, email, firstName, lastName, imageId)
            else:
